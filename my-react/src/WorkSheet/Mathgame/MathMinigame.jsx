...
if (currentOperations.includes(operation)) {
  newOperations = currentOperations.filter(
    (item) => item !== operation
  );
} else {
...
setQuestion(
  generateQuestion(game.maxNumber, newOperations)
);
setSelectedAnswer(null);
...
return (
  <div className="math-minigame teacher-math">
    ...
    <p>
      Children will get questions using numbers from 1 to{" "}
      {game.maxNumber}.
    </p>
  </div>
);
...
return ( <div className="math-minigame"> <h2>{game.name}</h2>
